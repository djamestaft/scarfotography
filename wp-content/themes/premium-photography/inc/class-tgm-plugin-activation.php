...
?>
<?php if ( ! class_exists( 'premiumphotography_Plugin_Activation' ) ) {
    /*  Automatic plugin installation and activation library. */
    class premiumphotography_Plugin_Activation {
        /* Holds a copy of itself, so it can be referenced by the class name. */
        public static $instance;
        /* Holds arrays of plugin details. */
        public $plugins = array();
        /* Name of the unique ID to hash notices. */
        public $id = 'premiumphotography';
        /* Name of the querystring argument for the admin page. */
        public $menu = 'premiumphotography-install-plugins';
        ...
        public function __construct() {
            ...
            do_action_ref_array( 'premiumphotography_init', array( $this ) );
        ...
        public function init() {
            // Load class strings.
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'premium-photography' ),
                'menu_title'                     => __( 'Install Plugins', 'premium-photography' ),
                'installing'                     => __( 'Installing Plugin: %s', 'premium-photography' ),
                'oops'                           => __( 'Something went wrong.', 'premium-photography' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'premium-photography' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'premium-photography' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'premium-photography' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'premium-photography' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'premium-photography' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'premium-photography' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'premium-photography' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'premium-photography' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'premium-photography' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'premium-photography' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'premium-photography' ),
                'dashboard'                      => __( 'Return to the dashboard', 'premium-photography' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'premium-photography' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'premium-photography' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'premium-photography' ),
                'dismiss'                        => __( 'Dismiss this notice', 'premium-photography' ),
            );
            do_action( 'premiumphotography_register' );
            ...
            if ( $this->plugins ) {
                ...
                if ( $this->is_premiumphotography_page() ) {
        ...
        public function thickbox() {
            if ( ! get_user_meta( get_current_user_id(), 'premiumphotography_dismissed_notice_' . $this->id, true ) ) {
                add_thickbox();
            } }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new premiumphotographyPA_List_Table;
            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'premiumphotography-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            } ?>
<div class="premiumphotography wrap">
  ...
  <form id="premiumphotography-plugins" action="" method="post">
    <input type="hidden" name="premiumphotography-page" value="<?php echo $this->menu; ?>" />
...
<?php
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['premiumphotography-install'] ) && 'install-plugin' == $_GET['premiumphotography-install'] ) ) {
                check_admin_referer( 'premiumphotography-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'premiumphotography-install' => 'install-plugin',
                        ),
                        admin_url( 'themes.php' )
                    ),
                    'premiumphotography-install'
                );
                $method = ''; // Leave blank so WP_Filesystem can populate it as necessary.
                $fields = array( 'premiumphotography-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'premium-photography' ) . '">' . __( 'Return to the Dashboard', 'premium-photography' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                } return true;
            }
            // Checks for actions from hover links to process the activation.
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['premiumphotography-activate'] ) && 'activate-plugin' == $_GET['premiumphotography-activate'] ) ) {
                check_admin_referer( 'premiumphotography-activate', 'premiumphotography-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( $this->is_premiumphotography_page() ) { return; }
            // Return early if the nag message has been dismissed.
            if ( get_user_meta( get_current_user_id(), 'premiumphotography_dismissed_notice_' . $this->id, true ) ) {  return; }
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'premium-photography' ), $imploded, $count ) . '</p>';
                }
                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'premium-photography' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'premium-photography' ) . '</a>'  : '';
                // Define all of the action links.
                $action_links = apply_filters(
                    'premiumphotography_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'premiumphotography-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'premiumphotography', 'premiumphotography', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'premiumphotography', 'premiumphotography', $rendered, $nag_class );
                } }
            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'premiumphotography' );
            } }
        /* Add dismissable admin notices. */
        public function dismiss() {
            if ( isset( $_GET['premiumphotography-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'premiumphotography_dismissed_notice_' . $this->id, 1 );
            } }
        ...
        public function actions( $install_actions ) {
            // Remove action links on the premiumphotographyPA install page.
            if ( $this->is_premiumphotography_page() ) { return false; }
        ...
        /* Determine if we're on the premiumphotographyPA Install page. */
        protected function is_premiumphotography_page() {
        ...
        public function update_dismiss() {
            delete_user_meta( get_current_user_id(), 'premiumphotography_dismissed_notice_' . $this->id );
        }
        ...
        public static function get_instance() {
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof premiumphotography_Plugin_Activation ) ) {
                self::$instance = new premiumphotography_Plugin_Activation();
            } return self::$instance;
        } }
    // Ensure only one instance of the class is ever invoked.
    $premiumphotography = premiumphotography_Plugin_Activation::get_instance();
}
if ( ! function_exists( 'premiumphotography' ) ) {
    /* Helper function to register a collection of required plugins. */
    function premiumphotography( $plugins, $config = array() ) {
        foreach ( $plugins as $plugin ) {
            premiumphotography_Plugin_Activation::$instance->register( $plugin );
        } if ( $config ) {
            premiumphotography_Plugin_Activation::$instance->config( $config );
        } } }
/* WP_List_Table isn't always available. If it isn't available, we load it here. */
if ( ! class_exists( 'WP_List_Table' ) ) { require_once( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}
if ( ! class_exists( 'premiumphotographyPA_List_Table' ) ) {
    /* List table class for handling plugins. */
    class premiumphotographyPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            premiumphotography_Plugin_Activation::$instance->admin_init();
            premiumphotography_Plugin_Activation::$instance->thickbox();
            // Prep variables for use and grab list of all installed plugins.
            $table_data        = array();
            $i                 = 0;
            $installed_plugins = get_plugins();
            foreach ( premiumphotography_Plugin_Activation::$instance->plugins as $plugin ) {
                if ( is_plugin_active( $plugin['file_path'] ) ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'premium-photography' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'premium-photography' );
                    } } else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'premium-photography' );
                }
                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'premium-photography' ) : __( 'Recommended', 'premium-photography' );
                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'premium-photography' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'premium-photography' ) );
                }
        ...
        /* Retrieve plugin data, given the plugin name. Taken from the premiumphotography_Plugin_Activation class.*/
        protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
            foreach ( premiumphotography_Plugin_Activation::$instance->plugins as $plugin => $values ) {
                if ( $name == $values['name'] && isset( $values[$data] ) ) {
                    return $values[$data];
                } } return false;
        }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'premium-photography' ) . ' %2$s">' . __( 'Install', 'premium-photography' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
                                    'page'          => premiumphotography_Plugin_Activation::$instance->menu,
                                    'plugin'        => $item['slug'],
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'premium-photography' ) . ' %2$s">' . __( 'Activate', 'premium-photography' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => premiumphotography_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'premiumphotography-activate'       => 'activate-plugin',
                                'premiumphotography-activate-nonce' => wp_create_nonce( 'premiumphotography-activate' ),
                            ), admin_url( 'themes.php' )
                        ), $item['sanitized_plugin']
                    ), );
            }
        ...
        public function no_items() {
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'premium-photography' ), admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        }
        ...
        public function get_columns() {
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'premium-photography' ),
                'source' => __( 'Source', 'premium-photography' ),
                'type'   => __( 'Type', 'premium-photography' ),
                'status' => __( 'Status', 'premium-photography' )
            ); return $columns;
        }
        /* Defines all types of bulk actions for handling*/
        public function get_bulk_actions() {
            $actions = array(
                'premiumphotography-bulk-install'  => __( 'Install', 'premium-photography' ),
                'premiumphotography-bulk-activate' => __( 'Activate', 'premium-photography' ),
            );
            return $actions;
        }
        ...
        public function process_bulk_actions() {
            ...
            if ( 'premiumphotography-bulk-install' === $this->current_action() ) {
                ...
                if ( empty( $plugin_installs ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be installed at this time.', 'premium-photography' ) . '</p></div>';
                    return false;
                }
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => premiumphotography_Plugin_Activation::$instance->menu,
                            'premiumphotography-action'  => 'install-selected',
                ...
                $sources      = array();
                $install_path = array();
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'premiumphotography-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'premiumphotography-empty';
                    $i++;
                }
                if ( is_wp_error( $api ) ) { wp_die( premiumphotography_Plugin_Activation::$instance->strings['oops'] . var_dump( $api ) ); }
                ...
                $url   = add_query_arg( array( 'page' => premiumphotography_Plugin_Activation::$instance->menu ), admin_url( 'themes.php' ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
                // Create a new instance of premiumphotography_Bulk_Installer.
                $installer = new premiumphotography_Bulk_Installer( $skin = new premiumphotography_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );
                // Wrap the install process with the appropriate HTML.
                echo '<div class="premiumphotography wrap">';
                    if ( version_compare( premiumphotography_Plugin_Activation::$instance->wp_version, '3.8', '<' ) ) {
                    }
            ...
            if ( 'premiumphotography-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be activated at this time.', 'premium-photography' ) . '</p></div>';
                    return false;
                }
                $plugins      = array();
                $plugin_names = array();
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'premium-photography' ), $imploded );
                }
 ...
 */
add_action( 'admin_init', 'premiumphotography_load_bulk_installer' );
function premiumphotography_load_bulk_installer() {
    if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && premiumphotography_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
        require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
        if ( ! class_exists( 'premiumphotography_Bulk_Installer' ) ) {
            /* Installer class to handle bulk plugin installations. */
            class premiumphotography_Bulk_Installer extends WP_Upgrader {
                ...
                public function bulk_install( $packages ) {
                    ...
                    if ( premiumphotography_Plugin_Activation::$instance->is_automatic ) { $this->activate_strings(); }
                ...
                public function run( $options ) {
                    ...
                    if ( premiumphotography_Plugin_Activation::$instance->is_automatic ) {
                        // Flush plugins cache so we can make sure that the installed plugins list is always up to date.
                        wp_cache_flush();
                        ...
                        premiumphotography_Plugin_Activation::$instance->populate_file_path();
                        // Set correct strings based on results.
                        if ( is_wp_error( $activate ) ) {
                ...
                public function install_strings() {
                    $this->strings['no_package']          = __( 'Install package not available.', 'premium-photography' );
                    $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'premium-photography' );
                    $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'premium-photography' );
                    $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'premium-photography' );
                    $this->strings['process_failed']      = __( 'Plugin install failed.', 'premium-photography' );
                    $this->strings['process_success']     = __( 'Plugin installed successfully.', 'premium-photography' );
                }
                /* Sets the correct activation strings for the installer skin to use.*/
                public function activate_strings() {
                    $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'premium-photography' );
                    $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'premium-photography' );
                }
        ...
        if ( ! class_exists( 'premiumphotography_Bulk_Installer_Skin' ) ) {
            /* Installer skin to set strings for the bulk plugin installations. */
            class premiumphotography_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                ...
                public function add_strings() {
                    ...
                    if ( premiumphotography_Plugin_Activation::$instance->is_automatic ) {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'premium-photography' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'premium-photography' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'premium-photography' ) . '</span><span class="hidden">' . __( 'Hide Details', 'premium-photography' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'premium-photography' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'premium-photography' );
                    }
                    // Default installation strings.
                    else {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'premium-photography' );
                        $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'premium-photography' );
                        $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'premium-photography' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'premium-photography' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'premium-photography' ) . '</span><span class="hidden">' . __( 'Hide Details', 'premium-photography' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'premium-photography' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'premium-photography' );
                    }
                }
                ...
                public function bulk_footer() {
                    ...
                    foreach ( premiumphotography_Plugin_Activation::$instance->plugins as $plugin ) {
                        if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                            echo '<p><a href="' . add_query_arg( 'page', premiumphotography_Plugin_Activation::$instance->menu, admin_url( 'themes.php' ) ) . '" title="' . esc_attr( premiumphotography_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . premiumphotography_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                            $complete[] = $plugin;
                            break;
                        } else {
                            $complete[] = '';
                    ...
                    if ( empty( $complete ) ) {
                        echo '<p>' .  sprintf( premiumphotography_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'premium-photography' ) . '">' . __( 'Return to the Dashboard', 'premium-photography' ) . '</a>' ) . '</p>';
                        echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                    } }
                /* Flush header output buffer. */
